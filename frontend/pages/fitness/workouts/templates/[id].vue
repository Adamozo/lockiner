...
useSeoMeta({
  title: 'Edit Template - LockIner',
})

const router = useRouter()
const route = useRoute()
const toast = useToast()
const { loading, fetchTemplate, updateTemplate } = useWorkoutTemplates()
...
onMounted(async () => {
  try {
    ...
    selectedType.value = template.workout_type as WorkoutType
    ...
    toast.add({ title: 'Error', description: 'Failed to load template', color: 'red' })
...
const handleSave = async () => {
  ...
  try {
    ...
    toast.add({ title: 'Template saved', color: 'green' })
    router.push('/fitness/workouts/templates')
  } catch {
    toast.add({ title: 'Error', description: 'Failed to save template', color: 'red' })
  }
}
</script>
...
<template>
  <div class="pb-4 max-w-2xl mx-auto">
    ...
    <header class="flex items-center gap-4 mb-6">
      ...
      <h1 class="text-2xl font-bold text-pure-white">Edit Template</h1>
    </header>
